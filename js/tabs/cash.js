...
function rCash(f){
  ...
  el.innerHTML=`
    ${srcN([srcP(BK_MAIN()+" → Dashboard_Data — рухи по рахунках",maxD(T,t=>t.ym)),BL.length?srcP(BK_ACC()+" → PRIVAT_BALANCES — поточні залишки"):""])}
    <div class="kpis">
      ...
      <div class="kpi"><div class="l">Всього $</div><div class="v" style="color:#10b981">${ff(totalUSD)} $</div></div>
    </div>
    <div class="kpis">
      <div class="kpi"><div class="l">₴ рахунок</div><div class="v">${ff(uahB)}</div></div>
  ...
  dc("cc1");CH.cc1=new Chart(document.getElementById("cc1"),{type:"bar",data:{labels:ms,datasets:[{label:"Вхідні",data:ms.map(m=>byM[m].i),backgroundColor:"#10b981",borderRadius:2},{label:"Вихідні",data:ms.map(m=>byM[m].o),backgroundColor:"rgba(239,68,68,.5)",borderRadius:2}]},options:{responsive:true,plugins:{legend:{labels:{color:"#7d8196",font:{size:9},boxWidth:9}}},scales:{x:{ticks:{color:"#7d8196",font:{size:7},maxTicksLimit:14},grid:{color:"#1e2130"}},y:{ticks:{color:"#7d8196",font:{size:9},callback:v=>fm(v)},grid:{color:"#1e2130"}}}}});
