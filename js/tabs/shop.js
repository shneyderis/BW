...
function rShop(f){
  ...
  const tabs=`<div class="sh-tabs">
    <button class="sh-tab ${shopSub==="sales"?"on":""}" onclick="shopSw('sales')">Продажі</button>
  ...
  ${srcN(srcP(BK_MAIN()+" → листи WC_Orders / WC_Products — інтернет-магазин WooCommerce (sync.gs)",maxD(WO,o=>o.date_created)))}`;
...
function rShopSales(el,tabs,orders,allOrd,f,c$){
  ...
  el.innerHTML=`${tabs}
    <div class="kpis" style="grid-template-columns:repeat(auto-fit,minmax(100px,1fr))">
      <div class="sh-kpi"><div class="l">Виручка</div><div class="v g">${ff(toCur(totalR))}${c$}</div>${cmpH(revGrow)}</div>
      <div class="sh-kpi"><div class="l">Замовлень</div><div class="v">${orders.length}</div>${cmpH(cntGrow)}</div>
      <div class="sh-kpi"><div class="l">Сер.чек</div><div class="v">${ff(toCur(avg))}${c$}</div>${cmpH(avgGrow)}</div>
      <div class="sh-kpi"><div class="l">ФОП податки</div><div class="v rd">${ff(toCur(fopCut))}${c$}</div><div class="s">${SETS.fopTax}%+${SETS.fopBank}%</div></div>
      <div class="sh-kpi"><div class="l">Чиста</div><div class="v g">${ff(toCur(totalR-fopCut))}${c$}</div></div>
    </div>
    <div class="cc"><h3>Виручка і замовлення помісячно</h3><canvas id="cs1" height="90"></canvas></div>
    <div class="row">
      <div class="cc"><h3>Середній чек (тренд)</h3><canvas id="csAvg" height="90"></canvas></div>
      <div class="cc"><h3>Виручка по тижнях (12 нед)</h3><canvas id="csW" height="90"></canvas></div>
    </div>
    <div class="row">
      <div class="cc"><h3>Способи оплати</h3>${payS.map(([m,v])=>{const pct=payTotal>0?(v/payTotal*100):0;return`<div style="display:flex;justify-content:space-between;align-items:center;font-size:10px;margin-bottom:4px"><span>${m}</span><span class="g" style="font-weight:600">${ff(toCur(v))}${c$} <span style="color:#7d8196;font-weight:400">(${pct.toFixed(0)}%)</span></span></div>`}).join("")}</div>
      <div class="cc"><h3>UTM джерела</h3><table class="tbl"><tr><th>Джерело</th><th class="r">Замовлень</th><th class="r">%</th></tr>${srcS.map(([s,c])=>`<tr><td>${s}</td><td class="r">${c}</td><td class="r" style="color:#7d8196">${(c/orders.length*100).toFixed(1)}%</td></tr>`).join("")}</table></div>
    </div>`;
  dc("cs1");CH.cs1=new Chart(document.getElementById("cs1"),{type:"bar",data:{labels:ms,datasets:[{label:"Виручка",data:ms.map(m=>toCur(byM[m].r)),backgroundColor:"#10b981",borderRadius:2,yAxisID:"y"},{label:"Замовлень",data:ms.map(m=>byM[m].c),type:"line",borderColor:"#f59e0b",pointRadius:2,pointBackgroundColor:"#f59e0b",borderWidth:1.5,yAxisID:"y1"}]},options:{responsive:true,plugins:{legend:{labels:{color:"#7d8196",font:{size:9},boxWidth:9}}},scales:{x:{ticks:{color:"#7d8196",font:{size:8}},grid:{color:"#1e2130"}},y:{position:"left",ticks:{color:"#7d8196",font:{size:9},callback:v=>fm(v)},grid:{color:"#1e2130"}},y1:{position:"right",ticks:{color:"#f59e0b",font:{size:9}},grid:{display:false}}}}});
...
function rShopProducts(el,tabs,orders,c$){
  ...
  if(hasLineItems){
    ...
    el.innerHTML=`${tabs}
      <div class="kpis" style="grid-template-columns:repeat(auto-fit,minmax(100px,1fr))">
        <div class="sh-kpi"><div class="l">Позицій продано</div><div class="v">${prodArr.length}</div></div>
        <div class="sh-kpi"><div class="l">Всього шт</div><div class="v">${ff(totalQ)}</div></div>
    ...
    if(catArr.length){dc("csCat");CH.csCat=new Chart(document.getElementById("csCat"),{type:"doughnut",data:{labels:catArr.map(([c])=>c),datasets:[{data:catArr.map(([,v])=>toCur(v)),backgroundColor:CC.concat(["#64748b","#0ea5e9","#d946ef"])}]},options:{responsive:true,plugins:{legend:{position:"bottom",labels:{color:"#7d8196",font:{size:8},boxWidth:8,padding:4}}}}})}
  } else {
...
function rShopCustomers(el,tabs,orders,c$){
  ...
  el.innerHTML=`${tabs}
    ...
    <div class="kpis" style="grid-template-columns:repeat(auto-fit,minmax(100px,1fr))">
      <div class="sh-kpi"><div class="l">Клієнтів</div><div class="v">${totalCust}</div></div>
      <div class="sh-kpi"><div class="l">Повторні</div><div class="v" style="color:#8b5cf6">${returning.length}</div><div class="s">${returningPct.toFixed(1)}%</div></div>
      <div class="sh-kpi"><div class="l">Сер. LTV</div><div class="v">${ff(toCur(avgLTV))}${c$}</div></div>
      <div class="sh-kpi"><div class="l">Сер. замовл./кл</div><div class="v">${avgOrders.toFixed(1)}</div></div>
    </div>
    <div class="cc"><h3>Нові клієнти vs Повторні замовлення</h3><canvas id="csCust" height="80"></canvas></div>
    <div class="row">
      <div class="cc"><h3>Топ клієнтів (LTV)</h3><table class="tbl"><tr><th>Клієнт</th><th class="r">LTV</th><th class="r">Зак.</th><th class="r">Місто</th></tr>${topLTV.map(c=>`<tr><td style="font-size:9px">${c.name.trim().substring(0,20)||c.email.substring(0,20)}</td><td class="r g">${ff(toCur(c.total))}${c$}</td><td class="r">${c.orders}</td><td class="r" style="color:#7d8196;font-size:9px">${c.city}</td></tr>`).join("")}</table></div>
      <div class="cc"><h3>Географія</h3><table class="tbl"><tr><th>Місто</th><th class="r">Кл.</th></tr>${topCities.map(([c,n])=>`<tr><td>${c}</td><td class="r">${n}</td></tr>`).join("")}</table></div>
    </div>`;
  dc("csCust");CH.csCust=new Chart(document.getElementById("csCust"),{type:"bar",data:{labels:cMs,datasets:[{label:"Нові",data:cMs.map(m=>firstByM[m]||0),backgroundColor:"#3b82f6",borderRadius:2},{label:"Повт.",data:cMs.map(m=>ordByM[m]?.ret||0),backgroundColor:"#8b5cf6",borderRadius:2}]},options:{responsive:true,plugins:{legend:{labels:{color:"#7d8196",font:{size:9},boxWidth:9}}},scales:{x:{stacked:true,ticks:{color:"#7d8196",font:{size:8}},grid:{color:"#1e2130"}},y:{stacked:true,ticks:{color:"#7d8196",font:{size:9}},grid:{color:"#1e2130"}}}}});
...
function rShopOrders(el,tabs,allOrd,completed,cancelled,refunded,pending,c$){
  ...
  const funnel=[{l:"Всього",v:totalOrd,c:"#7d8196"},{l:"Processing",v:(statuses["processing"]||0)+(statuses["completed"]||0)+(statuses["refunded"]||0),c:"#3b82f6"},{l:"Completed",v:statuses["completed"]||0,c:"#10b981"},{l:"Cancelled",v:statuses["cancelled"]||0,c:"#ef4444"},{l:"Refunded",v:statuses["refunded"]||0,c:"#ec4899"}];
  ...
  el.innerHTML=`${tabs}
    <div class="kpis" style="grid-template-columns:repeat(auto-fit,minmax(100px,1fr))">
      <div class="sh-kpi"><div class="l">Всього</div><div class="v">${totalOrd}</div></div>
      <div class="sh-kpi"><div class="l">Виконано</div><div class="v g">${completed.length}</div></div>
    ...
    <div class="row">
      ...
      <div class="cc"><h3>Статуси</h3><canvas id="csStatus" height="100"></canvas></div>
    </div>
    ...
    <div class="cc"><h3>По статусах (помісячно)</h3><canvas id="csOrdM" height="80"></canvas></div>
    <div class="cc"><h3>Останні замовлення</h3><table class="tbl"><tr><th>#</th><th>Дата</th><th>Клієнт</th><th class="r">Сума</th><th class="r">Статус</th></tr>${recent.map(o=>{const sc=statColors[o.status]||"#7d8196";return`<tr><td style="color:#7d8196">${o.id}</td><td style="font-size:9px">${(o.date_created||"").substring(0,10)}</td><td style="font-size:9px">${(o.billing?.first_name||"")} ${(o.billing?.last_name||"").substring(0,1)}.</td><td class="r g">${ff(toCur(parseFloat(o.total||0)))}${c$}</td><td class="r"><span style="color:${sc};font-weight:600;font-size:9px">${o.status}</span></td></tr>`}).join("")}</table></div>`;
