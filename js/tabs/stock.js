...
function rStock(){
  ...
  if(SD.length){
    ...
    el.innerHTML=`${tabs}${srcN(srcP(BK_MAIN()+" → лист Stock_Data — рухи та залишки складу",lastDate))}<div class="info">Stock_Data · до: ${lastDate} · ${wines.length} позицій</div>
    ...
    el.innerHTML=`${tabs}${srcN(srcP(BK_MAIN()+" → лист 3_Stock — залишки складу (старий формат)"))}<div class="info">3_Stock (старий). Stock_Data порожній.</div>
      <div class="row"><div>
...
function rStock1C(el,tabs){
  ...
  el.innerHTML=`${tabs}
    ${srcN(srcP("1С: файли data/1c_products.csv + data/1c_osv.csv (або книга "+BK_1C()+")"))}
