...
function rAssets(f){
  ...
  const yrs=aY();
  el.innerHTML=`
    <div class="kpis"><div class="kpi"><div class="l">Всього інвестицій</div><div class="v" style="color:#3b82f6">${ff(total)}${c$}</div></div>${ACATS.map(c=>'<div class="kpi"><div class="l">'+c.replace("Обладнання ","").replace(", власний транспорт","транспорт")+'</div><div class="v">'+ff(toCur(Math.abs(ft.filter(t=>t.cat.includes(c)).reduce((s,t)=>s+t.nt,0))))+c$+'</div></div>').join("")}</div>
    <div class="cc"><h3>Інвестиції по роках</h3><canvas id="ca1" height="130"></canvas></div>`;
