...
2. Через несколько секунд бот отвечает текстом.
...
3. **Деплой на Vercel.** Новый проект из этого репозитория, **Root Directory = `voice-bot`**. В Settings → Environment Variables добавить:
   ...
   Необязательно: `TRANSCRIBE_LANGUAGE=ru` (подсказка языка, по умолчанию автоопределение), `TRANSCRIBE_MODEL=whisper-1`.
