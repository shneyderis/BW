...
Пароли хранятся как SHA-256 хеши в `js/config.js` (открытым текстом нигде не хранятся). Смена пароля: вкладка ⚙ Настройки → «Смена пароля» → скопировать хеш в `config.js`.

| Роль | Доступ |
|---|---|
| **owner** | Все вкладки |
| **manager** | Продажи, Магазин, Склад, Маркетинг, Партнёры |
| **accountant** | P&L, Расходы, Осн.фонды, Кеш-фло, Партнёры, Нераспознанные |
